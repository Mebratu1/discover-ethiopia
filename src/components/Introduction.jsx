...
const FACTS = [
  ...
  { icon: '☕', label: 'Coffee',     desc: "Birthplace of coffee — the world's favourite beverage" },
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div ref={textRef} className="reveal grid md:grid-cols-2 gap-12 items-center mb-20">
          <div className="space-y-6">
            <p className="text-white/70 text-lg leading-relaxed font-light">
              ...
              took their first steps over{' '}
              <strong className="text-amber-400">3 million years ago</strong>.
            ...
            <p className="text-white/70 text-lg leading-relaxed font-light">
              Today, Ethiopia welcomes you into{' '}
              <strong className="text-amber-400">80+ distinct cultures</strong>,
              dramatic landscapes ranging from snow-capped peaks to active lava lakes, and a
              warmth of spirit that travellers call transformative. This is not sightseeing —
              this is homecoming.
            </p>
          </div>
          ...
          <div className="relative h-[400px] hidden md:block" aria-hidden="true">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 rounded-full bg-amber-500/10 border border-amber-500/30 flex items-center justify-center">
              <span className="text-3xl" role="img" aria-label="Ethiopian flag">🇪🇹</span>
            </div>
          </div>
        </div>
        ...
        <div ref={factsRef} className="stagger-children grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FACTS.map((fact) => (
            <div
              key={fact.label}
              className="border border-white/8 rounded-2xl p-6 bg-white/[0.03] hover:bg-white/[0.06] hover:border-amber-600/30 transition-all duration-300"
            >
              <div className="text-4xl mb-4" role="img" aria-label={fact.label}>{fact.icon}</div>
